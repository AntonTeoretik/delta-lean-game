...
## Контейнерная модель мира (текущий этап)

Текущая модель мира разделяет сущности на:

- **containers** (`FILE`, `NAMESPACE`, `SECTION`) — контекстные области
- **items** (`def`, `theorem`, ...) — редактируемые declaration-узлы
- **container metadata** — команды контекста (`variable`, `open`, `open scoped`, `universe`, `set_option`, ...)

Ключевые принципы:

- Узлы создаются только для declarations (`WorldItem`), а не для контекстных команд.
- `namespace` и `section` визуализируются как контейнеры (цветные области с заголовком и body).
- Координаты дочерних сущностей задаются **локально** относительно родительского контейнера.
- При перетаскивании сущность всегда ограничивается body-областью родителя (clamp).
- Неявный перенос в другой контейнер через drag-and-drop запрещён; только явные операции в будущих фазах.
